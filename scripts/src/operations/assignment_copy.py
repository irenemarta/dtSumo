...
def run_macroscopic_assignment(scenario: str, period: str, taz_file: Path) -> Path:
    ...
    routes_macro = run_marouter(
        ...
        netload_output=Path(work_dir, "netload_ouput2.xml"),
        method=METHOD,
        route_choice="gawron",
        gawron_beta=0.3,
        gawron_a=0.05,
        ...
        extra_args=["-l", str(work_dir / f"marouter_{METHOD}.log"), "--seed", "42"],
        weight_files=f"/home/fullsuper/irene/dtSumo/scripts/output/marouter-workdir/{scenario}_{period}/edge_output2.xml",
    )
    print(f"\t Macroscopic assignment saved here: {routes_macro}")

    edge_taz_map = build_edge_to_taz(cfg.TAZ)
...
def build_sumocfg(
    ...
    CfgAttributes(
        ...
        edgedata=Path(f"/home/fullsuper/irene/dtSumo/scripts/output/marouter-workdir/{scenario}_{period}/edgeData.xml")
    )
...
def run_macroscopic_assignment_day(
    ...
    routes_macro = run_marouter(
        ...
        #weight_files=f"/home/fullsuper/irene/dtSumo/scripts/output/marouter-workdir/{scenario}_DAY/edge_output.xml",
    )
...
def build_sumocfg_day(
    ...
    CfgAttributes(
        ...
        detectors=detectors_file,
        edgedata=Path(f"/home/fullsuper/irene/dtSumo/scripts/output/marouter-workdir/{scenario}_DAY/edgeData.xml")
